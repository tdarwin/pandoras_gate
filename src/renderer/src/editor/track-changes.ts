...
function visibleChanges(state: TrackState, doc: PMNode): Chunk[] {
  ...
  const chunks = undecided([...simplifyChanges(state.set.changes, doc)], state)
  state.chunks = chunks
  state.chunkDoc = doc
  return chunks
}
 ...
 * a word diff, and never rendered as chunks — see `blockShape.ts` for why.
 * Everything that puts a document on the overlay goes through here, so the
 * editor's content and the plugin's chain can never disagree about which
 * links are on screen.
 */
...
/** The markdown the editor holds for a spec: its last inline link, or the file. */
export function inlineDocContent(schema: PMNode['type']['schema'], spec: AttachSpec): string {
  const { chain } = inlineSpec(schema, spec)
  return chain[chain.length - 1]?.content ?? spec.original
}

function attachedState(schema: PMNode['type']['schema'], raw: AttachSpec, doc: PMNode): TrackState {
  const spec = inlineSpec(schema, raw)
  ...
  addCommands() {
    ...
    return {
      ...
      attachSuggestions:
        ...
        ({ state, tr, dispatch }) => {
          if (dispatch) {
            // The document becomes the last link that can be shown inline —
            // not the last link. A structural proposal is decided whole and
            // must not reach the screen as chunks, and a chain with nothing
            // inline in it leaves the document alone rather than emptying it,
            // which is what "the last link's content" meant when the fold had
            // set every proposal aside.
            const body = inlineDocContent(state.schema, spec)
            if (body !== docToMarkdown(state.doc)) {
              const next = markdownToDoc(state.schema, body)
        ...
        state: {
          init: (_config, state) => {
            ...
            const track = options.suggestion
              ? attachedState(state.schema, options.suggestion, state.doc)
              : inert
            return {
              ...track,
          ...
          apply: (tr, value, _old, newState) => {
            ...
            if (meta?.attach) {
              const track = attachedState(newState.schema, meta.attach, newState.doc)
