...
/** Each shown proposal with the entry changes it still proposes, the refused ones removed. */
function proposalsFor(
  active: ActiveSuggestions,
  entries: Entry[]
): { link: ActiveSuggestions['chain'][number]; changes: EntryChange[] }[] {
  return active.chain
    .map((link) => ({ link, entries: parseEntries(link.content) }))
    .filter((p): p is { link: ActiveSuggestions['chain'][number]; entries: Entry[] } => p.entries !== null)
    .map((p) => ({
      link: p.link,
      changes: diffEntries(entries, p.entries).filter((c) => !active.rejectedFields.includes(c.key))
    }))
}
...
export default function TimelineEditor({
  ...
  const proposals = useMemo(
    () => (active?.shown && entries !== null ? proposalsFor(active, entries) : []),
    [active, entries]
  )
  ...
  useEffect(() => {
    ...
    /**
     * Read the store LIVE, not this render's `proposals`.
     *
     * Rejecting an entry adds its key and persists in the same tick — before
     * React re-renders this component and re-registers the handle. A handle
     * closing over the render's memo therefore answered with the changes
     * filtered by the OLD rejected list, and the decision it sent said the
     * proposal still proposed the entry the author had just refused. The
     * refusal only became durable on some later save.
     */
    const live = (): ReturnType<typeof proposalsFor> => {
      const now = useProposalsStore.getState().active
      return now?.shown ? proposalsFor(now, entries) : []
    }
    setSuggestionHandle({
      // This view IS the overlay for a timeline: the cards are on screen for
      // as long as it is registered, so it can always speak for the proposals.
      suggestionsAttached: () => true,
      proposedBody: (proposalId) => {
        const p = live().find((x) => x.link.proposalId === proposalId)
        ...
        if (!p) {
          return (
            useProposalsStore.getState().active?.chain.find((l) => l.proposalId === proposalId)
              ?.content ?? value
          )
        }
      ...
      acceptAllSuggestions: () => {
        let next = entries
        for (const p of live()) next = p.changes.reduce(applyChange, next)
        onChange(serializeEntries(next))
      },
      rejectAllSuggestions: () => {
        for (const p of live()) for (const c of p.changes) rejectField(c.key)
      }
    })
    return () => setSuggestionHandle(null)
  }, [entries, onChange, rejectField, value])
